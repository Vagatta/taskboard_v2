...
export default function MyTasksPanel({ user }) {
  ...
  return (
    <div className="space-y-4">
      <Card className="border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/10 shadow-none">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-xs uppercase tracking-wide text-slate-500">Resumen personal</p>
            <p className="text-lg font-semibold text-slate-900 dark:text-white">Mis tareas abiertas</p>
          </div>
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-600 dark:text-slate-400">
            <div className="flex flex-col items-center text-center">
              <p className="text-slate-500">Total</p>
              <p className="text-base font-semibold text-slate-900 dark:text-white">{stats.total}</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <p className="text-slate-500">Pendientes</p>
              <p className="text-base font-semibold text-amber-700 dark:text-amber-200">{stats.pending}</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <p className="text-slate-500">Completadas</p>
              <p className="text-base font-semibold text-emerald-700 dark:text-emerald-300">{stats.completed}</p>
            </div>
          </div>
        </div>
      </Card>
      ...
      <Card className="border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-950/10 shadow-none">
        <div className="border-b border-slate-200 dark:border-slate-800 pb-2">
          <h3 className="flex items-center gap-2 text-sm font-medium text-cyan-600 dark:text-cyan-400">
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-950/10">
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm text-slate-900 dark:text-slate-100">
                  <thead className="bg-slate-50 dark:bg-slate-950/10 text-xs uppercase tracking-wide text-slate-600 dark:text-slate-400">
                    <tr>
                  ...
                  <tbody>
                    {filteredTasks.map((task) => {
                      ...
                      return (
                        <tr key={task.id} className="border-b border-slate-200 dark:border-slate-900/40 hover:bg-slate-50 dark:hover:bg-slate-900/10">
                          <td className="px-4 py-3">
                            <p className={`font-semibold ${task.completed ? 'text-slate-400 line-through' : 'text-slate-900 dark:text-white'}`}>
                              {task.title}
                            </p>
                          </td>
                          <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{projectLabel}</td>
                          <td className="px-4 py-3">
                            <Badge
                              color={
                                priority === 'high'
                          ...
                          <td className="px-4 py-3">
                            <span className={
                              dueDate && !task.completed && dueDate.getTime() < Date.now()
                                ? 'text-rose-600 dark:text-rose-200 font-semibold'
                                : 'text-slate-700 dark:text-slate-300'
                            }>
                              {dueLabel}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{updatedLabel}</td>
                        </tr>
                      );
                    })}
                  </tbody>
